...
for sub=1:length(subfile)
    ...
    path=fileparts(fileparts(file));  %保留上级目录
    folderpath=fullfile(path,name);   %添加下级目录
    mkdir(folderpath)   %在该路径建立文件夹
    path=fullfile(folderpath,'发展对比');%添加下级目录
    mkdir(path)       
    cd(path)
    ...
    data_rest1=fnirsfile.output.dc.dataTimeSeries(row1:row1+3300,:);     %%选取片段  165
    data_rest2=fnirsfile.output.dc.dataTimeSeries(row3:row3+3300,:);     %%选取片段  165
    data_stim1=fnirsfile.output.dc.dataTimeSeries(row2:row2+12980,:);   %%考虑往后1个周期误差   649
    
    %周期计算解释：
    %  20（刺激时长/分钟）*60（一分钟60秒）*11（秒内采样率）=13200
    %  20（20秒一个周期）*11（秒内采样率）=220
...
cd('C:\Users\luping\Desktop\数据_0.05\aver')
...
path='C:\Users\luping\Desktop\数据_0.05\aver\功率谱\';
mkdir(path)
cd(path)
