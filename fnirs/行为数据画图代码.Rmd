...
2.加载数据（行为实验2）
```{r}
data_inhi <- read.csv("F:/桌面文件夹/近红外论文/行为数据/实验一行为数据/inhi.csv")
data_nback <- read.csv("F:/桌面文件夹/近红外论文/行为数据/实验一行为数据/nback.csv")
data_shift <- read.csv("F:/桌面文件夹/近红外论文/行为数据/实验一行为数据/shift.csv")
```
...
# 更名
names(data_inhi1) <- c("stim.frequence","pre-stimulation","stimulating")
data_inhi1 <- pivot_longer(data = data_inhi1, cols = c("pre-stimulation", "stimulating"), names_to = "pre_post", values_to = "inhi_score")
data_inhi1$stim.frequence <- as.factor(data_inhi1$stim.frequence)
data_inhi1$pre_post <- factor(data_inhi1$pre_post, levels = c("pre-stimulation", "stimulating"))
...
plot11 <- ggplot(data_inhi1_summary, aes(x = pre_post, y = mean_score, group = stim.frequence, color = stim.frequence)) + 
  ...
  geom_point(data = data_inhi1, aes(x = pre_post, y = inhi_score, fill = stim.frequence), 
  ...
  theme(panel.grid.major = element_line(color = "gray", linetype = "dotted", size = 0.25), # 细化主网格线
        panel.grid.minor = element_blank())+ scale_color_manual(values = c("#F8766D", "#00BA38"))
...
ggsave("inhi1.jpg", plot11, width = 6, height = 4, units = "in", dpi = 300)
```
...
```{r}
 ...
 # 更名
names(data_nback1) <- c("stim.frequence","pre-stimulation","stimulating")
data_nback1 <- pivot_longer(data = data_nback1, cols = c("pre-stimulation", "stimulating"), names_to = "pre_post", values_to = "refresh_score")
data_nback1$stim.frequence <- as.factor(data_nback1$stim.frequence)
data_nback1$pre_post <- factor(data_nback1$pre_post, levels = c("pre-stimulation", "stimulating"))
...
```{r}
setwd("F:/桌面文件夹/近红外论文/图片")
ggsave("refresh1.jpg", plot22, width = 6, height = 4, units = "in", dpi = 300)
```

3.3 shift
```{r}
data_shift1 = data.frame(data_shift$stim.frequence, data_shift$相对转换代价b1, data_shift$相对转换代价b2) 
 # 更名
 names(data_shift1) <- c("stim.frequence","pre-stimulation","stimulating")
 data_shift1 <- pivot_longer(data = data_shift1, cols = c("pre-stimulation", "stimulating"), names_to = "pre_post", values_to = "switch_score")
 data_shift1$stim.frequence <- as.factor(data_shift1$stim.frequence)
 data_shift1$pre_post <- factor(data_shift1$pre_post, levels = c("pre-stimulation", "stimulating"))

plot3 <- ggplot(data_shift1, aes(x = stim.frequence, y = switch_score, group = interaction(stim.frequence, pre_post), fill = pre_post)) +
  geom_boxplot(width = 0.4, position = position_dodge(width = 0.5), color = "black", alpha = 0.8) +
  geom_point(position = position_dodge(width = 0.5), size = 3, color = "black", alpha = 0.6) +
...
data_shift_summary <- data_shift1 %>%
  group_by(stim.frequence, pre_post) %>%
  summarise(mean_score = mean(switch_score),
            sem_score = sd(switch_score) / sqrt(n()))

plot33 <- ggplot(data_shift_summary, aes(x = pre_post, y = mean_score, group = stim.frequence, color = stim.frequence)) + 
  geom_line(size = 1, position = position_dodge(width = 0.5)) +
  geom_point(data = data_shift1, aes(x = pre_post, y = switch_score, color = stim.frequence), 
             position = position_dodge(width = 0.5), shape = 21, fill = "white", size = 3) + 
  geom_errorbar(aes(ymin = mean_score - 3*sem_score, ymax = mean_score + 3*sem_score), width = 0.2, 
                position = position_dodge(width = 0.5), linetype = 1, size = 0.5) + # 细化误差线
...
```{r}
setwd("F:/桌面文件夹/近红外论文/图片")
ggsave("shift1.jpg", plot33, width = 6, height = 4, units = "in", dpi = 300)
```
