...
for sub= 1:length(dirInfo)
    ...
    subFolder=dir(fullfile(file,"*DICOM*"));
    file=fullfile(subFolder(1).folder,subFolder(1).name);

    subFolder=dir(file); 
    subFolder(1:2)=[];
    file=fullfile(subFolder.folder,subFolder.name);
    ...
    matlabbatch{1}.spm.util.import.dicom.outdir = {'F:\Yu_Hang\MRI\N'};
...
subpath = 'F:\Yu_Hang\MRI\N';
dirInfo = dir(subpath);
dirInfo(1:2)=[];

matchingFiles = dir(fullfile(subpath, '*000001*'));   %% 1005/1007
