clear
%% 1.FNIRS预处理
subpath='C:\Users\luping\Desktop\数据_0.05\snirf';
matpath='C:\Users\luping\Desktop\数据_0.05\homerOutput';
output_path='C:\Users\luping\Desktop\数据_0.05\result';
mkdir(output_path)
cd(output_path)
sampleFreq=11;
cyclicality_5=20;
Nrestmark_5=15;
Pretime=300;
[dcAvg_5,processdata_5]= yh_fnirs_process(subpath,matpath,sampleFreq,cyclicality_5,Nrestmark_5,Pretime);
save('dcAvg_5','dcAvg_5')
save('processdata_5','processdata_5')

%1.1数据分离
alldata=load('C:\Users\luping\Desktop\数据_0.05\result\dcAvg_5.mat');
output_path='C:\Users\luping\Desktop\数据_0.05\result\';
cd(output_path)

%1.1.1 stim
stim_data=alldata.dcAvg_5.stim;
for sub=1:size(stim_data,3)
 ...
 save('dcAvg_5_stim','stim')

%1.1.2 rest1
rest1_data=alldata.dcAvg_5.rest1;
for sub=1:size(rest1_data,3)
 ...
 save('dcAvg_5_rest1','rest1')

%1.1.3 rest2
rest2_data=alldata.dcAvg_5.rest2;
...
low_fre=load('C:\Users\luping\Desktop\数据_0.05\result\dcAvg_5.mat');
output_path='C:\Users\luping\Desktop\数据_0.05\result';
cd(output_path)

for sub=1:size(dcAvg_5.stim,3)
    for j=1:43
        Angle_O=hilbert(dcAvg_5.stim(:,1+3*(j-1),sub));
        Angle_R=hilbert(dcAvg_5.stim(:,2+3*(j-1),sub));
        Angle_T=hilbert(dcAvg_5.stim(:,3+3*(j-1),sub));
        % x_angle((end-9:end),:)=[];
        % x_angle(1:10,:)=[];
...
%2.2叠加平均后计算指标
%2.2.1 叠加后振幅
subfile='C:\Users\luping\Desktop\数据_0.05\result\dcAvg_5.mat';
path='C:\Users\luping\Desktop\数据_0.05\result';
fnirs_type=1;
...
%2.2.3 PSD:频率点活动强度
%功率谱
subfile='C:\Users\luping\Desktop\数据_0.05\result\processdata_5.mat';
fnirs_type=1;
aim_fre=0.05;
...
output_path='C:\Users\luping\Desktop\数据_0.05\result\';
cd(output_path)
load('dcAvg_5_stim.mat')
load('dcAvg_5_rest1.mat')
load('dcAvg_5_rest2.mat')
...
cd(output_path)
load('dcAvg_5_stim')
output_path='C:\Users\luping\Desktop\数据_0.05\result\GT\data\stim';
mkdir(output_path)
cd(output_path)
...
cd(output_path)
load('dcAvg_5_rest1')
output_path='C:\Users\luping\Desktop\数据_0.05\result\GT\data\rest1';
mkdir(output_path)
cd(output_path)
...
cd(output_path)
load('dcAvg_5_rest2')
output_path='C:\Users\luping\Desktop\数据_0.05\result\GT\data\rest2';
mkdir(output_path)
cd(output_path)
...
%叠加平均marker后的图
  ...
  cd(output_path)
  load('dcAvg_5.mat')
  dcAvg_5_rest1=mean(dcAvg_5.rest1,3);
  dcAvg_5_stim=mean(dcAvg_5.stim,3);
  dcAvg_5_rest2=mean(dcAvg_5.rest2,3);
  
  output_path='C:\Users\luping\Desktop\数据_0.05\result\图';
  mkdir(output_path)
  cd(output_path)
  for h=1:43
      x=1+3*(h-1);  %%  1=O 2=R 3=T
      plot(dcAvg_5_rest1(:,x),'r')
      hold on
      plot(dcAvg_5_stim(:,x),'g')
      plot(dcAvg_5_rest2(:,x),'b')
      title('叠加平均后血氧浓度变化')
      xlabel('时间窗')
      ylabel('血氧浓度')
...
output_path='C:\Users\luping\Desktop\数据_0.05\result\';
cd(output_path)
load('processdata_5.mat')
output_path='C:\Users\luping\Desktop\数据_0.05\result\图\PSD';
mkdir(output_path)
cd(output_path)

psd_stim = mean(processdata_5.stim,3);
psd_rest1 = mean(processdata_5.rest1,3);
psd_rest2 = mean(processdata_5.rest2,3);
noverlap=0;
nfft1=4096;
nfft2=16384;
Fs=11;
...
for h=1:43
    x=1+3*(h-1);  %%  1=O 2=R 3=T
    [pxx_stim,f_stim] = pwelch(psd_stim(:,x),hamming(12981),noverlap,nfft2,Fs);
