...
for sub=1:length(subfile)
    ...
    %滤波
    dod_mf=hmrR_BandpassFilt(dod_m,0.015,0.025);
    ...
    data_rest2=fnirsfile.output.dc.dataTimeSeries(row3:row3+3300,:);
    data_stim1=fnirsfile.output.dc.dataTimeSeries(row2:row2+12650,:);   %%考虑往后1个周期误差
    ...
    for h=1:43
        ...
        [pxx3,f3] = pwelch(data_stim1(:,x),hamming(12651),noverlap,nfft2,Fs);
        plot(f1,10*log10(pxx1),'r',f2,10*log10(pxx2),'b',f3,10*log10(pxx3),'g')
        title('Welch Power Spectral Density Estimate')
        xlabel('Normalized Frequency(×Π rad/sample)')
        ylabel('Power/frequency(dB/(rad/sample))')
        set(gca,'XLim',[0 0.2])
        line([0.02,0.02],[-150,50],'linestyle','--')
...
%% 保存
cd('C:\Users\luping\Desktop\数据_0.02\aver')
...
rest1_aver_O=zeros(3301,1);
rest2_aver_O=zeros(3301,1);
stim_aver_O=zeros(12651,1);
...
path='C:\Users\luping\Desktop\数据_0.02\aver\功率谱\';
mkdir(path)
cd(path)
...
[pxx3,f3] = pwelch(stim_aver_O(:,1),hamming(12651),noverlap,nfft2,Fs);
plot(f1,10*log10(pxx1),'r',f2,10*log10(pxx2),'b',f3,10*log10(pxx3),'g')
title('Welch Power Spectral Density Estimate')
xlabel('Normalized Frequency(×Π rad/sample)')
ylabel('Power/frequency(dB/(rad/sample))')
set(gca,'XLim',[0 0.2])
line([0.02,0.02],[0,140],'linestyle','--')
legend('rest1','rest2','stim')
...
[pxx3,f3] = pwelch(stim_aver_R(:,1),hamming(12651),noverlap,nfft2,Fs);
plot(f1,10*log10(pxx1),'r',f2,10*log10(pxx2),'b',f3,10*log10(pxx3),'g')
title('Welch Power Spectral Density Estimate')
xlabel('Normalized Frequency(×Π rad/sample)')
ylabel('Power/frequency(dB/(rad/sample))')
...
[pxx3,f3] = pwelch(stim_aver_T(:,1),hamming(12651),noverlap,nfft2,Fs);
plot(f1,10*log10(pxx1),'r',f2,10*log10(pxx2),'b',f3,10*log10(pxx3),'g')
title('Welch Power Spectral Density Estimate')
xlabel('Normalized Frequency(×Π rad/sample)')
ylabel('Power/frequency(dB/(rad/sample))')
...
for i=1:43
    ...
    [pxx3,f3] = pwelch(stim_aver(:,x),hamming(12651),noverlap,nfft2,Fs);
    plot(f1,10*log10(pxx1),'r',f2,10*log10(pxx2),'b',f3,10*log10(pxx3),'g')
    title('Welch Power Spectral Density Estimate')
    xlabel('Normalized Frequency(×Π rad/sample)')
    ylabel('Power/frequency(dB/(rad/sample))')
